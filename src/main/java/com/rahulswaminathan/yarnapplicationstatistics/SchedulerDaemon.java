...
class SchedulerThread implements Runnable {
    ...
    Map<JsonElement, String> queuesToParentsMap;
        ...
        while (running) {
            try {
                ...
                //            "steadyFairResources,fairResources,clusterResources");
                    ...
                    queuesToParentsMap = getAllQueuesToParentsForCapacityScheduler(jo);
    ...
    private void initializeQueuesTables(Map<JsonElement, String> queuesToParentsMap) {
        for (JsonElement e : queuesToParentsMap.keySet()) {
            ...
            mySqlWrapper.dropTable(queueName);
            ...
            if (obj.has("queues")) {
                ...
                mySqlWrapper.createParentQueueTable(queueName + "_");
                insertIntoParentQueueTable(queueName, e);
            } else {
                ...
                mySqlWrapper.createChildQueueTable(queueName + "_");
                insertIntoChildQueueTable(queueName, e);
            }
        }
    }

    private void insertIntoParentQueueTable(String queueName, JsonElement e) {
        JsonObject obj = e.getAsJsonObject();
        if (queueName.equals("root")) {
            return;
        }
        mySqlWrapper.insertIntoQueueTable(queueName + "_", obj.get("capacity").getAsString(), obj.get("maxCapacity").getAsString(),
                obj.get("numApplications").getAsString(), obj.get("absoluteCapacity").getAsString(), obj.get("absoluteMaxCapacity").getAsString(),
                obj.get("absoluteUsedCapacity").getAsString(), queuesToParentsMap.get(e), String.valueOf(System.currentTimeMillis()));
    }

    private void insertIntoChildQueueTable(String queueName, JsonElement e) {
        JsonObject obj = e.getAsJsonObject();
        mySqlWrapper.insertIntoQueueTable(queueName + "_", obj.get("capacity").getAsString(), obj.get("maxCapacity").getAsString(),
                obj.get("numApplications").getAsString(), obj.get("absoluteCapacity").getAsString(), obj.get("absoluteMaxCapacity").getAsString(),
                obj.get("absoluteUsedCapacity").getAsString(), queuesToParentsMap.get(e), String.valueOf(System.currentTimeMillis()),
                obj.get("maxApplications").getAsString(), obj.get("maxApplicationsPerUser").getAsString(), obj.get("numContainers").getAsString(),
                obj.get("numPendingApplications").getAsString(), obj.get("resourcesUsed").toString(), obj.get("state").getAsString(),
                obj.get("usedCapacity").getAsString(), obj.get("userLimit").getAsString(), obj.get("users").toString(), obj.get("type").getAsString(),
                obj.get("userAMResourceLimit").toString(), obj.get("usedAMResource").toString(), obj.get("AMResourceLimit").toString(),
                obj.get("userLimitFactor").getAsString(), obj.get("preemptionDisabled").getAsString(), obj.get("hideReservationQueues").getAsString(),
                obj.get("nodeLabels").toString());
    }
